...
class Sale_lib
{
	...
	function add_payment( $payment_id, $payment_amount, $cantidad_recibida )
	{
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null)
	{
        ...
        function add_membership($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null)
	{
		$items = $this->get_cart();
		...
		array(
			'item_id'=>$item_id,
			'line'=>$insertkey,
			'name'=>$this->CI->Member->get_info($item_id)->NOMBRE_CLIENTE,
			'item_number'=>$this->CI->Member->get_info($item_id)->NUMERO_MEMBRESIA,
			'description'=>$description!=null ? $description: $this->CI->Member->get_info($item_id)->DESCRIPCION_ADEUDO,
			'serialnumber'=>$serialnumber!=null ? $serialnumber: '',
			'allow_alt_description'=>0,
			'is_serialized'=>0,
			'quantity'=>$quantity,
                        'discount'=>$discount,
			'price'=>$price!=null ? $price: $this->CI->Member->get_info($item_id)->MONTO_ADEUDO
			)
		);
